...
这是一个课程设计项目，目标是实现一个可运行的智能文档问答平台。系统支持上传 PDF、TXT、Word 文档，后端会调用 Python RAG 服务解析文档、切分文本、生成向量并写入向量库；用户可以在前端围绕已上传文档进行问答，回答支持 Markdown 与 LaTeX 渲染，并保留对话历史和引用片段。

## 功能概览

- 文档上传：支持 PDF、TXT、DOCX、DOC。
...
3. 进入“上传文档”，上传 PDF、TXT 或 Word 文件。
